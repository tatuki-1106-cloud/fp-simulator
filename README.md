...
共通スケジュール画面では、収入・支出・一般口座積立を年齢と年月の範囲、毎月・毎年・一回払い、年率上昇率で登録できます。Q9のローンは年月ごとの変動金利、Q6の住宅は管理費・修繕積立金・固定資産税などの期間別コスト、Q10の保険は月払・年払・複数年払・一括払に対応します。Q11の一般口座には年齢・年月ベースの積立と運用利回りを設定できます。

「FP UNIVインポート」では、円/万円単位のJSONを反映前にプレビューできます。既存メンバーへのマッピング、追加・更新内容、未対応項目や単位換算などの近似警告を表示し、確認後に保存します。税額・制度・金融商品の実額を保証するものではないため、最終判断には各機関の情報を利用してください。
